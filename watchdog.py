...
def setup_logging(base_dir: Path, run_timestamp: str) -> tuple[logging.Logger, logging.Logger]:
    """Configure watchdog and child-output loggers with timestamped files."""
    ...
    # Timestamped watchdog log file
    watchdog_log_file = base_dir / f"watchdog-{run_timestamp}.log"
    watchdog_file = logging.FileHandler(watchdog_log_file, encoding="utf-8")
    ...
    # Timestamped server output log file
    server_log_file = base_dir / f"server-output-{run_timestamp}.log"
    output_file = logging.FileHandler(server_log_file, encoding="utf-8")
...
class Watchdog:
    ...
    def __init__(
        ...
        # Generate timestamp for this watchdog run
        self.run_timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        self.watchdog_logger, self.output_logger = setup_logging(log_dir, self.run_timestamp)
    ...
    def run(self) -> int:
        ...
        self.watchdog_logger.info("Session logs:")
        self.watchdog_logger.info("  - Watchdog: watchdog-%s.log", self.run_timestamp)
        self.watchdog_logger.info("  - Server output: server-output-%s.log", self.run_timestamp)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-dir",
        type=Path,
        default=Path(os.getenv("WATCHDOG_LOG_DIR", "logs/watchdog")),
        help="Directory where watchdog logs and crash reports are written. (env: WATCHDOG_LOG_DIR)",
    )
    parser.add_argument(
        "--restart-delay",
        type=float,
        default=float(os.getenv("WATCHDOG_RESTART_DELAY", "5.0")),
        help="Seconds to wait before restarting after a crash. (env: WATCHDOG_RESTART_DELAY)",
    )
    parser.add_argument(
        "--max-restarts",
        type=int,
        default=int(os.getenv("WATCHDOG_MAX_RESTARTS", "0")),
        help="Maximum number of restart attempts before giving up (0 = unlimited). (env: WATCHDOG_MAX_RESTARTS)",
    )
    parser.add_argument(
        "--crash-tail-lines",
        type=int,
        default=int(os.getenv("WATCHDOG_CRASH_TAIL_LINES", "200")),
        help="How many recent output lines to include in crash reports (0 = disable). (env: WATCHDOG_CRASH_TAIL_LINES)",
    )
    parser.add_argument(
        "--restart-on-success",
        action="store_true",
        default=os.getenv("WATCHDOG_RESTART_ON_SUCCESS", "").lower() in ("true", "1", "yes"),
        help="Restart even when the supervised process exits with code 0. (env: WATCHDOG_RESTART_ON_SUCCESS)",
    )
    parser.add_argument(
        "--label",
        default=os.getenv("WATCHDOG_LABEL", "lloegrys"),
        help="Short label used in crash report filenames. (env: WATCHDOG_LABEL)",
    )
    parser.add_argument(
        "command",
